...
> **⛔ 项目完结（2026-07-22）**：平台开发已停更、对公司业务不再推进。送别与历程见 **[`完结.md`](完结.md)**。下文仅作考古 / 运维备忘，**勿再按现役产品迭代。**

> **战略变更（2026-07-21 · 停更前最后一次）**：**Hunyuan 已下线**；曾拟转 Seedance / GPT Image——**未及落地即完结**。旧画布 `modelId=hunyuan-*` 降级逻辑仍留在代码中。

用于开启新对话时快速恢复上下文。最后更新：**2026-07-22**（完结入土）
...
| **画风统一-2** | 画质增强 preset 映射 | ✅ | `recommend-params` 读 `defaultQualityPresetId`；`enhanceProfile=cinematic` → noise 0.15 |
| **画风统一-3** | 视频卡画风二合一 | ✅ | `VideoStylePicker` 替换原「风格参考」钮；`video-gen.data.qualityPresetId`；生成传 `quality_preset_id` |
| **画风统一-4** | VideoStylePicker 触发器 UI | ✅ | 视频卡 + **分镜表工具栏** + **镜头卡** 均 `IconQualityStyle` + `画风：{name}`；`showUploadSection={false}` 复用组件 |
| **画风统一-5** | 清理重复入口 | ✅ | 删分镜表「内容风格」、图像卡 PromptBar 画风 UI；`contentStylePresets.js` 已删；旧数据迁移 |
...
出图/视频统一用 `computeScriptTableGenX()` / `computeScriptTableShotY()`；**加载画布时** [`migrateGenNodePositions.js`](frontend/src/utils/canvas/migrateGenNodePositions.js) 会对符合旧公式 `table.x + 1360 + gap` 的 `image-gen` / `video-gen` / `script-beat-card` **自动重定位 X**（启发式，容差 24px）；其余旧节点仍须手拖或重新生成。

### 图像卡上传（Phase 44 速查）

| 行为 | 说明 |
|------|------|
| 工具栏「上传图片」 | 若当前选中**空** `image-gen` → 写入该卡 `uploadedImage`；否则新建卡（**不再**与已有卡叠在同一中心点） |
| 新建 `image-gen` | 有选中节点时落点在**其右侧**；无选中时中心点**阶梯偏移**避重叠（`resolveCreateNodePosition`） |
| 纯上传卡状态 | `status: "input"`（非 `completed`）；已有 `uploadedImage` 时不显示顶部 upload banner |
| 数据字段 | `uploadedImage` 持久化于画布 JSON；新建其它卡**不会**清除其它节点上的上传图 |

### 当前产品形态速查

| 能力 | 粒度 / 入口 | 存储 | 注入范围 |
|------|-------------|------|----------|
| **画风预设** | 分镜表「默认画风」+ 镜头卡「画风」；视频卡 `VideoStylePicker` | `defaultQualityPresetId` + 行 `qualityPresetId`；视频卡 `video-gen.data.qualityPresetId` | build-shot / 视频 prompt suffix；增强推荐读表级默认 |
| **视频风格参考** | 镜头级；视频卡 `VideoStylePicker` 下半「上传参考视频」 | `video-gen.data.styleReference`（canvas JSON） | **仅视频生成**（与 preset suffix 可叠加） |
...
## 详情：Phase 44 — 画布 UI 十四项优化 + 上传/加载页修补（2026-07-03 · ✅ 已实现）

**标注：画布交互与视觉一致性大轮次；未改后端 API 契约**

### A. 基础设施

| 项 | 说明 |
|----|------|
| Portal 工具 | [`themePortal.js`](frontend/src/utils/themePortal.js) + [`themePortalRoot.js`](frontend/src/utils/themePortalRoot.js)：`getThemePortalRoot()` + `getThemePageClass()`，避免 portal 丢深色 token |
| z-index | [`zIndexLayers.js`](frontend/src/utils/zIndexLayers.js)：评论 / 下拉 / 节点 chrome 等分层常量 |
| 共享 chrome | [`CanvasShared.css`](frontend/src/components/canvas/CanvasShared.css) `.chrome-pill-bar` |
| 下拉互斥 | [`canvasDropdownCoordinator.js`](frontend/src/components/canvas/canvasDropdownCoordinator.js)：`closeActiveCanvasDropdown()`；接入 CastMentionPicker、ScriptTable、ScriptShotCard、PromptBar 等 |
| 动效基线 | [`motion.css`](frontend/src/styles/motion.css)；[`AnimatedModal.jsx`](frontend/src/components/common/AnimatedModal.jsx)（部分 Modal 进出场） |

### B. Portal 批量迁移（~22 处）

`document.body` → `getThemePortalRoot()` + 主题 fallback 类；涉及 Workspace 下拉、@ 列表、评论菜单、模型 dropup、VideoStylePicker、ShareMenu、各类 Modal 等。

### C. 上传 banner（图像卡 / 视频卡）

| 迭代 | 结果 |
|------|------|
| 曾试 Portal + fixed + `motion-popover-in` | 动画 transform 覆盖定位，与标签行错位 → **已弃用** |
| **当前** | `gn2-upload-row` 作为 `gn2-wrapper` **首子元素**；CSS `position:absolute; transform:translate(-50%, calc(-100% - 12px))`；`opacity/visibility` 显隐 |
| 显示条件 | `isRefSource \|\| (isIdle && !uploadedImage && !showResultsGrid && selected)` |
| 文件 | `GenerationCardNode.jsx/css`、`VideoGenerationNode.jsx` |

### D. 评论与选中态

| 项 | 说明 |
|----|------|
| 锚点 | `getCommentAnchorRect()` / `getCommentPinPosition()` 统一 panel 与 pin（`commentMarkerLayout.js`） |
| 角标 | hover tip 左右 flip；pin `focus-visible` 键盘；`CommentAuthorPreview` 点头像资料浮窗 |
| 选中态 | outline / beat-card / image-upload 与 `CanvasShared` focused 规则对齐 |
| TextNode | 删 `tn-mode-row`；选中时隐藏粘贴剧本横幅；模式切换清 hint |

### E. 旧画布节点 X 坐标迁移

[`migrateGenNodePositions.js`](frontend/src/utils/canvas/migrateGenNodePositions.js)：加载时在 `useCanvasSave.applyCanvasData` 调用；仅当 X 符合旧公式 `table.x + 1360 + gap` 且 Y 匹配行/节拍时才改为 `computeScriptTableGenX()`。

### F. 图像卡上传叠层修复（用户反馈）

| 问题 | 根因 | 修复 |
|------|------|------|
| 新建图片卡后「上传图消失」 | 工具栏新建卡与已上传卡**同坐标叠放**，新卡 z-index 更高盖住旧卡 | `resolveCreateNodePosition()`：选中时在右侧；否则中心阶梯偏移 |
| 上传写到错误卡 | 工具栏「上传图片」总新建卡 | 选中空 `image-gen` 时 `onUpdate({ uploadedImage })` |
| 纯上传误标完成态 | `status:"completed"` 且无 results | 改为 `status:"input"`（`handleUploadImage` / flyout drop / drag place） |

关键文件：`nodeHelpers.js`（`resolveCreateNodePosition`、`isEmptyImageGenNode`）、`useCanvasNodes.js`、`useCanvasInteraction.js`、`useCanvasDragPlace.js`。

### G. Velora 刷新加载页

[`VeloraLoadingPage.jsx/css`](frontend/src/components/common/VeloraLoadingPage.jsx)：移除中间脉冲圆环（`logo-ring`）；保留 Logo 旋转 + 「Velora」字标 + 「正在连接服务器…」；用于 `ProtectedRoute` / Login / Register 首屏。

### H. Workspace / 评论二次修复（同批次）

| 项 | 说明 |
|----|------|
| 上传剧本 Tab | 去 `min-height:300px` / dropzone `flex:1`，高度随内容 |
| 左栏 hover | 恢复默认 pill；统一 portal + `z-index:10060` |
| 视频 Prompt Bar | 删除每次输入自动展开首尾帧；仅 `@` 图片时展开 |
| @ 列表 | `video-at-mention` / `cast-mention-picker` / 评论 compose → `top:100%` 向下 |
| 评论 | MessageMenu / inline 提及列表回滚；角标 hover 头像 → 资料预览 |

### I. 浏览器验收 checklist（Phase 44）

- [ ] 刷新 / 登录首屏：仅旋转 Logo + 字标 + 连接文案，**无中间圆环**
- [ ] 选中空图像卡上传 → 再点「图片生成」：旧卡上传图仍在左侧/旁侧可见
- [ ] 工具栏「上传图片」：选中空卡写入当前卡；无选中时新建且不叠在已有卡上
- [ ] 选中图像/视频 idle 卡：upload banner 浮在整卡（含 Image/Video 标签）上方
- [ ] 深色/亮色：Workspace 与画布 portal 下拉背景正确
- [ ] 分镜表与镜头卡下拉：同时只开一个
- [ ] 打开旧项目：分镜表右侧出图列 X 若符合旧公式应自动贴近表缘

**关键文件（Phase 44 新增/主改）**：

```
frontend/src/utils/themePortal.js
frontend/src/utils/themePortalRoot.js
frontend/src/utils/zIndexLayers.js
frontend/src/utils/canvas/migrateGenNodePositions.js
frontend/src/utils/canvas/nodeHelpers.js              # resolveCreateNodePosition, isEmptyImageGenNode
frontend/src/components/canvas/canvasDropdownCoordinator.js
...
frontend/src/components/common/VeloraLoadingPage.jsx|css
frontend/src/hooks/canvas/useCanvasInteraction.js
frontend/src/hooks/canvas/useCanvasSave.js
...
## 详情：Phase 43 — 服务器部署前收尾验收（2026-07-03 · ✅ 已完成）
...
### C. 探针（19 × `scripts/_*_probe.py`）

前置：Redis 开；`alembic upgrade head`（**022**）；后端 `:7788` + **`AGENT_MOCK_GENERATION=true`**（覆盖 `.env` 中 `false`）；`SEED_ADMIN_PASSWORD` 已设。
...
| `_style_reference_probe.py` | 0 | |
| `_excel_import_probe.py` | 0 | 无本地样例 xlsx 时仅分组逻辑 |
| `_lut_probe.py` | 0 | |
...
| `_agent_pipeline_probe.py` | **2** | deprecated 调试脚本；需传账号参数 |
| `_comfyui_workflow_structure_probe.py` | **3** | 本地 ComfyUI 可达但缺权重/自定义节点；**延至服务器 GPU 环境** |
| `_adversarial_regression_probe.py` | **3** | LLM 非确定性（本轮最佳 **5/6**；末次 4/6） |
| `_agent_pipeline_e2e_probe.py --skip-text` | **3** | LLM 阶段顺序偶发偏差（R5/R8b）；**`_mock_pipeline_stage2_probe` 已覆盖 mock 主链路** |
...
警告（非阻塞）：`uploadImage.js` 动/静态 import 重复；主包 **>500 kB**（既有 chunk 体积提示）。

### E. 部署文档与 env

| 文件 | 变更 |
|------|------|
| [`DEPLOY.md`](DEPLOY.md) | 迁移 **001–022**；`uploads/luts/`；`assets/luts/`；LUT API 表；ffmpeg 含 LUT |
| [`backend/docs/AUTODL_DEPLOY_RUNBOOK.md`](backend/docs/AUTODL_DEPLOY_RUNBOOK.md) | 同上 + `mkdir uploads/luts` |
| [`backend/docs/COMFYUI_CUTOVER_RUNBOOK.md`](backend/docs/COMFYUI_CUTOVER_RUNBOOK.md) | 迁移 022 |
| [`backend/.env.example`](backend/.env.example) | `DEEPSEEK_API_KEY`、`AGENT_MODEL`、`CANVAS_LOCK_*`、`UVICORN_ACCESS_LOG`、探针变量注释 |

### F. 上服务器前仍待环境项（非代码阻塞）

| 项 | 说明 |
|----|------|
| ComfyUI 全模型结构探针 | 服务器配齐权重 + WanVideoWrapper 后重跑 `_comfyui_workflow_structure_probe.py` |
| 真实 GPU 全链路 | `_real_media_pipeline_probe.py` + HANDOFF **第八节 B** |
| LLM 回归探针 | `_adversarial_regression_probe` / `_agent_pipeline_e2e` 对 LLM 方差敏感；部署前可各重跑 1 次 |

---

## 详情：画风预设统一 + 视频卡 VideoStylePicker（2026-07-02 · ✅ 已实现）

### A. 后端 — `quality_preset_id` 单一来源

| 项 | 说明 |
|----|------|
| Preset 注册表 | [`quality_presets.py`](backend/services/quality_presets.py)：9 个 id（含 `auto` + 8 种画风）；`suffix` / `enhanceProfile` |
| Prompt 注入 | [`prompt_builder.py`](backend/services/prompt_builder.py)：`quality_preset_id` 注入正/负向 suffix；**不再**读 `contentStyle` |
| 视频任务 | [`tasks.py`](backend/routers/tasks.py)：`CanvasVideoRequest.quality_preset_id`；trace 字段同步 |
| 画质增强 | [`video_enhance_recommend.py`](backend/services/video_enhance_recommend.py)：`enhanceProfile=cinematic` → `input_noise_scale=0.15` |
| 画布读取 | [`canvas_style_ref.py`](backend/services/canvas_style_ref.py) → `get_script_table_default_quality_preset` |
| 迁移 | 旧 `contentStyle=photorealistic_cinema` + `defaultQualityPresetId=auto` → `cinematic`；mount 时清除 `contentStyle` |
| 测试 | `test_quality_presets.py` + `test_prompt_builder.py` 改写为 preset 驱动断言 |

### B. 前端 — 入口收敛与视频卡二合一

| 项 | 说明 |
|----|------|
| Preset 数据 | [`scriptQualityPresets.js`](frontend/src/utils/canvas/scriptQualityPresets.js)：与后端 id 对齐；含暗调戏剧/冷峻都市/胶片年代等 |
| 分镜表 | [`ScriptTableNode.jsx`](frontend/src/components/canvas/ScriptTableNode.jsx)：`VideoStylePicker`（`showUploadSection={false}`）设表级默认；[`ScriptShotCard.jsx`](frontend/src/components/canvas/ScriptShotCard.jsx) 镜头卡同款触发器 |
| 图像卡 | [`CanvasPromptBar.jsx`](frontend/src/components/canvas/CanvasPromptBar.jsx)：**不加**画风选择（产品原则） |
| 视频卡 | [`VideoStylePicker.jsx`](frontend/src/components/canvas/VideoStylePicker.jsx) + [`VideoReferencePanel.jsx`](frontend/src/components/canvas/VideoReferencePanel.jsx) |
| 生成链路 | [`useScriptTableGenerate.js`](frontend/src/hooks/canvas/useScriptTableGenerate.js)、[`VideoGenerationNode.jsx`](frontend/src/components/canvas/VideoGenerationNode.jsx) 传 `quality_preset_id` |
| 工具函数 | [`scriptTableNode.js`](frontend/src/utils/canvas/scriptTableNode.js)：`getEffectiveQualityPresetId`、`resolveVideoQualityPresetId` |
| 已删除 | `contentStylePresets.js` |

**VideoStylePicker 模式**：
- **视频卡**（`showUploadSection=true`）：预设列表 + 上传/管理参考视频
- **分镜表 / 镜头卡**（`showUploadSection=false`）：仅预设列表，触发器样式与视频卡一致

### C. 触发器 UI 规则（三处一致）

| 规则 | 说明 |
|------|------|
| 文案 | `IconQualityStyle` + `画风：{预设名}`（`canvas.stylePicker.tagValue`）；视频卡有参考视频时追加 `· 参考` |
| 默认态 | `auto`（且无参考视频）：`--text-secondary`、不加粗、背景透明 |
| hover / 展开 / 非 auto | 加粗 + 主色；hover 时有灰底 |
| 无下拉箭头 | 已移除 `▾` chevron |

**浏览器验收 checklist（画风统一）**：

- [ ] 分镜表工具栏：`[icon] 画风：由模型自己选择`（无外侧「默认画风」标签）
- [ ] 镜头卡头部：同款 `画风：XXXX` 触发器（无外侧「画风」标签）
- [ ] 图像卡 PromptBar 无画风下拉
- [ ] 视频卡触发器：`[icon] 画风：由模型自己选择`（默认较淡、不加粗）
- [ ] 选中预设或上传参考后：加粗；hover 有灰底；无右侧箭头
- [ ] 生成视频请求 payload 含 `quality_preset_id`（非 auto 时）
- [ ] `npm run build` 通过；`pytest tests/test_quality_presets.py tests/test_prompt_builder.py` 通过

**关键文件**：

```
backend/services/quality_presets.py
backend/services/prompt_builder.py
backend/services/video_enhance_recommend.py
backend/services/canvas_style_ref.py
backend/routers/tasks.py, routers/prompt.py
backend/schemas/tasks.py, schemas/prompt_builder.py
backend/tests/test_quality_presets.py
backend/docs/STYLE_PRESET_UNIFICATION.md

frontend/src/utils/canvas/scriptQualityPresets.js
frontend/src/utils/canvas/scriptTableNode.js
frontend/src/components/canvas/VideoStylePicker.jsx|css
frontend/src/components/canvas/VideoReferencePanel.jsx
frontend/src/components/canvas/VideoAtMentionList.jsx
...
frontend/src/components/canvas/VideoGenerationNode.jsx
frontend/src/components/canvas/CanvasTopbarIcons.jsx          # IconQualityStyle
frontend/src/hooks/canvas/useScriptTableGenerate.js
frontend/src/utils/locale.js                                # canvas.stylePicker.*
```
...
| ~~P2~~ | ~~画风预设统一 + 视频卡 VideoStylePicker~~ | ✅ 2026-07-02 |
| ~~P2~~ | ~~Phase 43 部署前收尾（探针/build/env/文档 022）~~ | ✅ 2026-07-03 |
...
│   ├── VideoStylePicker.jsx|css, VideoAtMentionList.jsx
...
```bash
cd backend && alembic upgrade head   # 001–022（022 = tasks.lut_applied）
# REDIS_URL 必配 — 未开 Redis 则协作/Agent/生成槽位/限流异常
# DASHSCOPE_API_KEY 必填（Agent / 文本 / classify / 风格参考 VL）
# uploads/ 持久化（images / videos / exports / luts）
# backend/assets/luts/*.cube 内置 LUT 随代码部署（勿删）
```

**LUT API**（[`routers/lut.py`](backend/routers/lut.py)）：`GET/PUT /api/projects/{id}/lut` · `POST /api/lut/upload` · `POST .../lut/apply-all` · `POST /api/tasks/video-lut`
...
| **42** | **07-02** | **画风预设统一**：`quality_preset_id` 替代 `contentStyle`；视频卡 `VideoStylePicker` 二合一 + 触发器 UI（✅） |
| **43** | **07-03** | **部署前收尾**：全量 pytest + 19 探针 + build + `.env.example` + 文档 022/LUT；`tasks.py` settings 修复（✅） |
| **44** | **07-03** | **画布 UI 十四项**：Portal/z-index/下拉互斥、upload banner、评论锚点、选中态、`migrateGenNodePositions`、上传卡叠层修复、Velora 加载页去圆环 + Workspace/评论二次修复（✅） |
