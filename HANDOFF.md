...
用于开启新对话时快速恢复上下文。最后更新：**2026-06-30**（V1 探针覆盖 + 主题圆形扩散 · 均已验收）

---

## 当前总览（2026-06-30 · 最新）

本轮在 **Backlog v2 + 分镜表 UX 四轮** 基础上，连续完成 **V1 画布探针覆盖补全**（7 个新探针 + `tasks/records` 路由修复）与 **主题切换圆形扩散过渡**（View Transitions API）；上述功能均已 **浏览器 / 探针双端验收通过**。后端单测 **42 passed**，前端 `npm run build` 通过。
...
| 项 | 说明 |
|----|------|
| `CanvasShareMenu` | Portal 下拉；复制链接 / 导出 / 邀请 / 只读（后两项占位） |
...
左栏工具栏（`CanvasLeftToolbar`）：工具钮 + 底部头像；头像上方 **半横线**；头像菜单含账户管理 / 管理后台（admin）/ **切换画布主题**（`useThemeTransition`）/ 登出。工作区顶栏、资料弹窗亦有主题钮，三入口动效一致。
...
│   ├── CanvasLeftToolbar.jsx
│   ├── GenerationCardNode.jsx             # 加号拖线参考实现
...
```bash
cd backend && .venv\Scripts\python.exe -m alembic upgrade head   # 011～017
# DASHSCOPE_API_KEY 必填；可选 AGENT_MODEL=deepseek-v3
# COMFYUI_URL 指向内网 ComfyUI；切换真实模型见 backend/docs/COMFYUI_CUTOVER_RUNBOOK.md
# 无 ComfyUI 时可选：AGENT_MOCK_GENERATION=true（阶段二 E2E 测试）
# AGENT_MOCK_FAILURE_RATE=0.1  # 可选，测失败 UX
# uploads/ 持久化；Redis 必开
```
...
```
继续 AI Studio 开发。请先读 HANDOFF.md 文首「2026-06-30 当前总览」+ [`V1_CANVAS_PROBE_COVERAGE.md`](backend/docs/V1_CANVAS_PROBE_COVERAGE.md) + 「占位功能 — 永久不实现」。

【当前状态 · 2026-06-30】
- Backlog v2 + 分镜表 UX 四轮（✅ 浏览器验收通过）
- V1 探针覆盖补全（✅）：7 个新探针 + tasks/records 路由修复；文档见 backend/docs/
- 主题切换圆形扩散（✅）：useThemeTransition；三入口；与 scope 动画独立
- Phase 9～36 + 37b + 探针 + 主题过渡均已交付
- 粘贴剧本：仅**文本卡**走 classify；图/视频直接生成
- 生成历史：个人 localStorage / 团队 GET /api/tasks/records（路由已修复）
...
【下轮可选 · 产品/工程】
1. Admin 分流：为各文本模型填单价后实测「低价优先」「均衡分流」
2. 批量出图前逐格预置构图参考（低频，约 0.5–1 天）
3. 公网前：APP_ENV=production、强 JWT（见 SECURITY_AUDIT_FINDINGS.md）
...
【前端改完】Ctrl+Shift+R；改 tasks.py / style_reference 相关后须重启 uvicorn
```
