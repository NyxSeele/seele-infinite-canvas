...
（不等 GPU 推理）。用于在算力不足时提前发现 HiDream / Wan 等 workflow 结构性问题。
...
前置：在 ComfyUI models/ 下人工放置同名占位权重（几 KB 空文件即可，勿由本脚本创建/覆盖）：
  ...
  diffusion_models/ flux1-dev-fp8.safetensors, hidream_i1_dev_fp8.safetensors, wan2.2_t2v_*_14B_fp8_scaled.safetensors
  text_encoders/ clip_l_hidream.safetensors, clip_g_hidream.safetensors, t5xxl_fp8_e4m3fn_scaled.safetensors,
                 llama_3.1_8b_instruct_fp8_scaled.safetensors, umt5_xxl_fp8_e4m3fn_scaled.safetensors
  loras/         wan2.2_t2v_lightx2v_4steps_lora_v1.1_*.safetensors
  SEEDVR2/       seedvr2_ema_3b_fp8_e4m3fn.safetensors, ema_vae_fp16.safetensors
  Wan 2.2 使用原生 ComfyUI 节点链（非 WanVideoWrapper）。
...
PROBE_I2V_IMAGE = "03_video_wan2_2_14B_i2v_subgraphed_input_image.png"
...
PROBE_TARGETS: dict[str, ProbeTarget] = {
    ...
    "flux-pulid": ProbeTarget("flux-pulid", "flux-pulid", baseline=False, category="image"),
    ...
    "wan-i2v": ProbeTarget("wan-i2v", "wan-i2v", baseline=False, category="video"),
    "wan-flf2v": ProbeTarget("wan-flf2v", "wan-i2v", baseline=False, category="video"),
    "wan-fun-inpaint": ProbeTarget(
        "wan-fun-inpaint", "wan-fun-inpaint", baseline=False, category="video"
    ),
...
def build_workflow_for_target(target: ProbeTarget) -> dict:
    ...
    if target.key == "flux-pulid":
        ckpt = _checkpoint_for(target.registry_id)
        profile = resolve_generation_profile(target.registry_id, ckpt)
        return _build_flux_pulid_workflow(
            PROBE_PROMPT,
            ckpt,
            width_img,
            height_img,
            PROBE_SEED,
            profile,
            reference_face_image="probe_face.jpg",
            pulid_weight=0.8,
        )
    ...
    if target.key == "wan-i2v":
        return build_wan_i2v_workflow(
            PROBE_PROMPT,
            PROBE_NEGATIVE,
            PROBE_I2V_IMAGE,
            width=width_vid,
            ...
            duration_sec=PROBE_DURATION_SEC,
            seed=PROBE_SEED,
            model_filename=WAN22_I2V_HIGH,
        )

    if target.key == "wan-flf2v":
        return build_wan_flf2v_workflow(
            PROBE_PROMPT,
            PROBE_NEGATIVE,
            PROBE_I2V_IMAGE,
            PROBE_I2V_IMAGE,
    ...
    if target.key == "wan-fun-inpaint":
        return build_wan_fun_inpaint_workflow(
            PROBE_PROMPT,
            PROBE_NEGATIVE,
            PROBE_I2V_IMAGE,
            PROBE_I2V_IMAGE,
            ...
            duration_sec=PROBE_DURATION_SEC,
            seed=PROBE_SEED,
            model_filename=WAN22_FUN_INPAINT_HIGH,
        )

    if target.key == "ltx2-fp4":
