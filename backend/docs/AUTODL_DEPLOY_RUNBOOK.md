...
| hunyuan-video-1.5 | 视频 | ~27 GB（含 VAE/TE） | ~31 GB 峰值 | ready |
...
> **硬约束 · 系统盘**：`/` 默认仅 **30GB**。除系统包、Redis/Nginx/Supervisor 等极小配置外，**严禁**将模型权重、pip/npm/conda 缓存、临时测试文件、大日志、git 大仓库、生成产物等无关内容写入系统盘。一律放 **`/root/autodl-tmp`**（数据盘，可扩容）。`TMPDIR` 须指向 `/root/autodl-tmp/tmp`；大目录可软链至 `/root/autodl-tmp/.sys-mirror/`。操作前 `df -h /` 确认 **<80%**。

| 路径 | 大小 | 用途 |
|------|------|------|
| `/` 系统盘 | 30 GB（默认） | **仅**系统包、Redis、Nginx、Supervisor 配置；**禁止**放大文件 |
| `/root/autodl-tmp` | **扩容 250–300 GB+** | ComfyUI、模型权重、`uploads/`、SQLite、日志、缓存、miniconda、临时目录 |
| `/root/autodl-tmp/.sys-mirror/` | — | 从系统盘迁出的 miniconda、`.cache`、`.npm`、LaunchTool311 等（软链回 `/root/`） |
...
│   ├── backend/
│   │   ├── assets/luts/      # 内置 LUT 预设 .cube（随代码部署，勿删）
│   │   ├── uploads/          # 必须在此数据盘下（images/videos/exports/luts）
...
├── videos/
├── exports/
└── luts/              # 用户上传自定义 .cube，按 project_id 分子目录
```
...
| `ffmpeg` | 风格参考抽帧 + **全片 LUT** 后处理（`video_lut_service`；`imageio-ffmpeg` 可选） |
| `alembic upgrade head` | 迁移 **001–022**（含 export_jobs、llm_routing、style_reference、**lut_applied=022**） |
...
mkdir -p uploads/images uploads/videos uploads/exports uploads/luts
```
...
`/api/uploads/videos/*` 由 Nginx `sendfile` 直接读盘返回，**不经后端转发文件体**。鉴权通过内部子请求 `GET /api/media/auth-check`（校验 `?mt=` / Bearer + 资源归属）。前端 URL 格式不变。
...
cp /root/autodl-tmp/AIStudio/deploy/nginx-autodl.conf /etc/nginx/sites-available/aistudio
nginx -t && nginx -s reload
# 若 backend 尚未包含 /api/media/auth-check：supervisorctl restart aistudio-backend
```
...
- [ ] `alembic upgrade head` 无报错（**022**）
- [ ] `backend/assets/luts/` 存在 6 个内置 `.cube`；`uploads/luts/` 可写
