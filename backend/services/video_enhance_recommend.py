...
- model_size: "3b" | "7b"（H800 默认 7b FP16 顶配）
- strength: "normal" | "sharp"（AI 成片默认 normal，避免 oversharpen；实拍糊片可用 sharp）
- model_variant: 固定 "fp16"

推荐逻辑：
- 分辨率短边 ≤720 → upscale_factor 2.0；720–1080 → 1.5；已 ≥1080 → 1.0
- source_type 为 ai_generated → input_noise_scale 0.25、strength normal
...
def normalize_enhance_params(raw: dict[str, Any] | None) -> dict[str, Any]:
    ...
    model_size = str(data.get("model_size") or default_seedvr_model_size()).strip().lower()
    if model_size not in VALID_MODEL_SIZE:
        model_size = default_seedvr_model_size()
...
def _rule_recommend_params(
    ...
    *,
    ...
    default_size = default_seedvr_model_size()
    ...
    size_label = "7B FP16" if params["model_size"] == "7b" else "3B FP8"
    prefix = "写实电影 AI 推荐：" if cinematic else "AI 推荐："
    reasoning = f"{prefix}{reason_scale}；时长 {duration:.1f}s 批次 {batch}；SeedVR2 {size_label}"
