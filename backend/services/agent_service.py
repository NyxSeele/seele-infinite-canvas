...
_MANIFEST = load_pipeline("velora_canvas")
_PIPELINE_FORCE_STEPS = get_force_steps(_MANIFEST)
_PIPELINE_KNOWN_STEPS = get_all_steps(_MANIFEST)
_PIPELINE_ADVANCE_STAGES = _PIPELINE_KNOWN_STEPS | frozenset({
    "wait",
    "wait_outline",
    "wait_script_table",
    "wait_storyboard",
...
_STAGE_TABLE_FULL = build_prompt_stage_table(_MANIFEST, slim=False)
_STAGE_TABLE_SLIM = build_prompt_stage_table(_MANIFEST, slim=True)
_SKILLS_FULL = build_skills_prompt_section(_MANIFEST, include_stages=True, slim=False)
_SKILLS_SLIM = build_skills_prompt_section(_MANIFEST, include_stages=True, slim=True)
_STEP_ENUM = build_step_enum(_MANIFEST)
...
- 角色待配图：`cast_pending` 为角色名数组，配合 `script_table_id`；options 含「现在配图」「先跳过，继续生成」
- 场景待配图：`scene_pending` 为场景名数组；value 可用 assign_scene_images / skip_scene_images
- group_title / group_subtitle 可选；有 options 时建议填写
- options 可选，2～4 个创意卡片，用于故事方向/风格路线等**非剧本主链路**讨论
- 有 ask_user 时，done.summary **≤2 句**；禁止 options 里写英文 step 名
...
- 链路执行类（意图B）：summary 描述本步结果；**必须**给 2～3 个 suggestions
- 分析类（意图A）：summary 按结构写完整分析；suggestions 可选
- 创意策划（意图D，有 ask_user.options）：summary ≤2 句；**不要** suggestions
...
7. 阶段二按镜号单线程：每次只推进**一镜**的一个子步骤；当前镜视频完成前禁止下一镜。
8. 多 `script_table` 时模糊指代必须先 ask_user 澄清。
...
→ user_status 写 1～3 行进度（导演语感）
→ done.summary 按结构写完整中文分析（整体状态 → 逐镜 → 缺什么/下一步）
→ **禁止**只回复「是/否」或说「分镜表为空」当 rows_summary 有数据

### B. 链路执行 / 创作推进
用户说：继续、下一步、生成、创建、采纳……**或用户已选定创意方案（如「我选择…」）** **或用户明确要求另开新链路**
→ 按链路进度提示执行**一个** pipeline_step；开新链路时执行 create_text_note（**新建**节点）
→ **禁止**在用户首次提出全新主题、尚未选定方向时直接 create_text_note（走意图 D）
→ **ask_user 门禁**：最近 assistant 含 ask_user 且用户仅说推进词 → 禁止推进，done 等待选择
→ **新明确方向**：用户给出新的明确创作内容 → 放弃旧 ask_user，按 B/D 处理

### C. 普通闲聊
→ 可只 done 回复，或 create_text_note（chat 模式）

### D. 创意策划 / 方向讨论（非剧本强制链路）
用户说：几个方向、故事路线、人物设定、风格选择、帮我想想……
**或**第一次描述创作主题且尚未选定方案、也未说推进词
→ **禁止** pipeline_step / create_text_note
→ 用 ask_user + options（2～4 个中文卡片），或只 done 给建议
→ 用户选定卡片后下一轮走意图 B 执行 create_text_note
→ 画布上已有其他主题节点时，禁止推进旧节点
...
用户：「继续」（分镜表已有 rows，镜1 分镜图已完成，推荐 generate_video）
...
def _resolve_agent_model() -> tuple[str, str, str, str | None]:
    ...
    if ep.get("source") == "fallback" or not ep.get("api_key") or not ep.get("base_url"):
        return "", "", "", None

    model = ""
    if row:
        model = (row.model_string or row.id or "").strip()
    if not model:
        model = (ep.get("default_model") or "").strip()
    if not model:
        return "", "", "", None

    reg_id = row.id if row else None
    return ep["api_key"], ep["base_url"], model, reg_id
...
def _drop_unknown_pipeline_steps(actions: list) -> list:
    """Reject pipeline_step actions whose step is not declared in the manifest."""
    if not actions:
        return actions
    filtered: list = []
    dropped: list[str] = []
    for action in actions:
        if action.get("type") != "pipeline_step":
            filtered.append(action)
            continue
        step = (action.get("step") or "").strip()
        if step and step not in _PIPELINE_KNOWN_STEPS:
            dropped.append(step)
            logger.warning("unknown pipeline_step rejected: %s", step)
            continue
        filtered.append(action)
    if not dropped:
        return filtered
    has_mutating = any(a.get("type") in MUTATING_ACTION_TYPES for a in filtered)
    has_done = any(a.get("type") == "done" for a in filtered)
    if not has_mutating and not has_done:
        filtered.append({
            "type": "done",
...
async def run_agent_stream(request: AgentRunRequest) -> AsyncGenerator[str, None]:
    ...
    actions = _drop_unknown_pipeline_steps(actions)
