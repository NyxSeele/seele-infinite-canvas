...
### 阶段二 — 分镜制作（分镜表就绪后，**一镜一步**）

| 步骤 | step 值 | 前置条件 |
|------|---------|----------|
| 5 | split_shot_beats | 某镜尚无节拍（rows_summary.has_beats=false） |
| 6 | generate_storyboard | 该镜已有节拍、分镜图未完成 |
| 7 | generate_video | 该镜分镜图已全部完成、尚无**已完成**的视频（has_video=false） |
| 8 | 循环下一镜 | 按镜号顺序重复 5→6→7，直至全镜完成 |

**禁止**在阶段二未完成时说「剧本链路已完成」！分镜表生成后应引导「为镜 1 拆分节拍」。
...
7. 阶段二每次只推进**一镜**的一个子步骤（节拍 / 出图 / 出视频）。
...
def _resolve_agent_model() -> tuple[str, str, str, str | None]:
    ...
    if not api_key:
        api_key = (settings.dashscope_api_key or os.environ.get("DASHSCOPE_API_KEY") or "").strip()
    model = (settings.agent_model or "deepseek-v3").strip()
    return api_key, DASHSCOPE_BASE_URL, model, None
...
def _production_stage_hint(script_table: dict) -> tuple[str, str]:
    """分镜制作阶段：按镜推进大分镜直连出图→视频（节拍拆分为可选）。"""
    st_id = script_table.get("id", "")
    rows = script_table.get("rows_summary") or []
    if not rows:
        return (
            "wait_script_table",
            "分镜表尚无镜头行，请等待分镜表生成完成",
