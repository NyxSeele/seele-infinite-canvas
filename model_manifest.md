...
> 热模型合计：**约 149G**（按 10MB/s ≈ **4.1 小时**）

## 热模型（常驻数据盘，每次必须有）

对应 registry `enabled=True`：`flux-dev` / `hidream` / `wan-2.6` / `wan-i2v` / `wan-fun-inpaint` / `ltx2-fp4` / `video-enhance-seedvr2` / `image-enhance-seedvr2`。
...
| seedvr2_ema_7b_fp16（默认顶配） | `/root/autodl-tmp/ComfyUI/models/SEEDVR2/seedvr2_ema_7b_fp16.safetensors` | ~16.5G | hf-mirror `numz/SeedVR2_comfyUI` |
| seedvr2_ema_7b_sharp_fp16 | `/root/autodl-tmp/ComfyUI/models/SEEDVR2/seedvr2_ema_7b_sharp_fp16.safetensors` | ~16.5G | 同上（strength=sharp） |
...
4. 按需跑温模型：`backend/scripts/_download_g30_ltx2_weights.sh` / `_download_g39_audiogen.sh` / `_download_g40_buffalo_l.sh`
5. 重启 ComfyUI 与后端
